...
export class HeartbeatRunner {
  ...
  async runOnce(config, force = false) {
    ...
    try {
      ...
      // turn 级单步超时配置（dsh-turn-guard 读取）：从配置读 stepTimeoutMs，
      ...
      const stepMs = Number(config.stepTimeoutMs) > 0 ? Number(config.stepTimeoutMs) : 0;
      if (agent?.__pluginConfig) {
        if (stepMs > 0) agent.__pluginConfig.turnGuard = { stepMs };
