...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function HeartbeatSection(props) {
      const { getConfig, setConfig, trigger } = props;
      ...
      const save = () => {
        Promise.resolve().then(() => setConfig({
          ...
          quietStart: cfg.quietStart, quietEnd: cfg.quietEnd,
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("settings.section", () => ctx.slots.register(
        { name: "settings.section", id: "heartbeat", order: 25, label: () => "心跳", inject: () => ({ getConfig, setConfig, trigger }) },
        HeartbeatSection,
      ));
    }
